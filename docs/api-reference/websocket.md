...
Use whichever host and port your API is running on. An alias endpoint is also available at `/v1/ws/live`.

!!! info "Authentication"
    WebSocket connections require an `X-API-Key` header with a key that has `readAccess`. Connections without a valid key are rejected with close code `1008 (Policy Violation)`. See [API Key Management](../getting-started/api-keys.md).
