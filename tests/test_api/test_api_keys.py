...
class TestApiKeyBootstrap:
    """Tests for POST /v1/api-keys/bootstrap."""

    @pytest.mark.asyncio
    async def test_bootstrap_creates_initial_key(self, client: AsyncClient):
        """Bootstrap succeeds when no active keys exist."""
        response = await client.post("/v1/api-keys/bootstrap")
    ...
    @pytest.mark.asyncio
    async def test_bootstrap_blocked_when_active_key_exists(self, client: AsyncClient):
        """Bootstrap returns 403 when at least one active key already exists."""
        await _create_key(client)
        ...
        data = response.json()
        assert data["errorCode"] == 403

    @pytest.mark.asyncio
    async def test_bootstrap_allowed_after_all_keys_deactivated(self, client: AsyncClient):
        """Bootstrap is permitted again once all active keys have been deactivated."""
        key = await _create_key(client)
        await client.delete(f"/v1/api-keys/{key['id']}")
