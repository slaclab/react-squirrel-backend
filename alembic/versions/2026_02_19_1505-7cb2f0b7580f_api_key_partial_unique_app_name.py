...
down_revision: str | None = "c4797e015c4b"
...
def upgrade() -> None:
    ...
    op.create_index(
        "uq_api_key_app_name_active", "api_key", ["app_name"], unique=True, postgresql_where=sa.text("is_active = TRUE")
    )
...
def downgrade() -> None:
    ...
    op.create_index(
        ...
        [
            sa.literal_column(
                "to_tsvector('english'::regconfig, (((((COALESCE(setpoint_address, ''::character varying)::text || ' '::text) || COALESCE(readback_address, ''::character varying)::text) || ' '::text) || COALESCE(device, ''::character varying)::text) || ' '::text) || COALESCE(description, ''::text))"
            )
        ],
        unique=False,
    ...
    op.create_unique_constraint(
        op.f("api_key_app_name_key"), "api_key", ["app_name"], postgresql_nulls_not_distinct=False
    )
