...
async def get_all_tag_groups(service: TagService = Depends(get_tag_service)) -> list[TagGroupSummaryDTO]:
...
async def get_tag_group(group_id: str, service: TagService = Depends(get_tag_service)) -> list[TagGroupDTO]:
...
async def create_tag_group(data: TagGroupCreate, service: TagService = Depends(get_tag_service)) -> TagGroupDTO:
