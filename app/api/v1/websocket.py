...
class DiffStreamManager:
    ...
    1. Track subscriptions per client
    ...
    async def start(self) -> None:
        ...
        logger.info(f"DiffStreamManager started (batch interval: {self._batch_interval_ms}ms)")
    ...
    async def stop(self) -> None:
        ...
        logger.info("DiffStreamManager stopped")
...
def get_diff_manager() -> DiffStreamManager:
    """Get or create the diff stream manager singleton."""
    global _manager
    if _manager is None:
        _manager = DiffStreamManager()
    return _manager
